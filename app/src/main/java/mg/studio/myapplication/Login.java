...
public class Login extends AppCompatActivity {
    ...
    class OnlineCredentialValidation extends AsyncTask<String, Void, Integer> {
        ...
        protected Integer doInBackground(String... strings) {
            ...
            final String stremail=strings[0];
            final String strpassword=strings[1];
...
//parameter 7: sorting method
            Cursor cursor = db.query("stu_table", new String[]{"email","password"},
                    "email=?", new String[]{stremail}, null, null, null);
            while(cursor.moveToNext()){
                String password = cursor.getString(cursor.getColumnIndex("password"));
                if(password.equals(strpassword)){
                    db.close();
                    feedback.setName(stremail);
                    return feedback.SUCCESS;
                }
            }
            db.close();
            feedback.setName(stremail);
            feedback.setError_message("You email or password is error, pleace check them!");
